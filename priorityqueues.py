# From Michael Markl: https://github.com/Schedulaar/predicted-dynamic-flows/blob/main/predictor/src/utilities/queues.py

import heapq
from typing import Optional, List, TypeVar, Generic, Tuple, Dict, Hashable
from utilities import ExtendedRational
...
class PriorityQueue(Generic[T]):
    ...
    _data: List[Tuple[ExtendedRational, int, T]]
    _index_dict: Dict[T, int]
    _index: int

    def __init__(self, initial: Optional[List[Tuple[T, ExtendedRational]]] = None):
        if initial:
    ...
    def push(self, item: T, key: ExtendedRational):
    ...
    def key_of(self, item: T, default: Optional[ExtendedRational] = None) -> ExtendedRational:
        if item not in self._index_dict:
            return default
        return self._data[self._index_dict[item]][0]

    def decrease_key(self, item: T, key: ExtendedRational):
    ...
    def min_key(self) -> ExtendedRational:
        return ExtendedRational(1,0) if len(self._data) == 0 else self._data[0][0]
    ...
    def update(self, item: T, new_key: ExtendedRational):
    ...
    def set(self, item: T, new_key: ExtendedRational):
        if self.has(item):
            self.update(item, new_key)
        else:
            self.push(item, new_key)

    def increase_key(self, item: T, new_key: ExtendedRational):
        assert self.has(item)
        pos = self._index_dict[item]
    ...
    def remove(self, item: T):
        ...
        self._data[pos] = (ExtendedRational(-1,0), -1, item)
        self._siftdown(0, pos)
